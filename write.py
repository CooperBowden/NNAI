...
class WriteMixin:
    ...
    async def awrite_stream(
        ...
        async for chunk in stream.async_response_gen():  # type: ignore
            if chunk == "Empty Response":
                # If no response is generated, do not write and just return so it can be handled
                return chunk
            # Replace $ to avoid latex formatting
            chunk = chunk.replace("$", "\$")
            ...
            if isinstance(chunk, str):
                ...
                # Use st.text rather than st.markdown to avoid latex formatting
